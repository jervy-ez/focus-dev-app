...
class Invoice extends MY_Controller{
	...
	public function list_invoiced_items($project_id,$project_total,$variation_total){
		...
		foreach ($fetch_project_jobbook_raw->result() as $row) {
			...
			if($row->label == ''){
				...
				if($row->label == 'VR'){
					...
					echo ($row->is_paid == 1 ? '&#9745; Paid' : '');
					echo '</strong> &nbsp;   <strong class="pull-right"> EX-GST &nbsp; &nbsp; &nbsp; : $'.number_format($variation_total).'</strong></p>';
					$set_var_total = 1;
		...
		echo '<p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p><hr /><p><strong class="pull-right">Total EX-GST  &nbsp; &nbsp; : $'.number_format($total+$variation_total).'</strong></p>';
	}
