		...
		if($project_details->num_rows > 0){
			...
			if($project_comments->num_rows > 0){
				...
				foreach ($project_comments->result_array() as $row){
					...
					echo '<div class="notes_line"><p class="">'.ucfirst ($row['project_comments']).'</p><small><i class="fa fa-user"></i> '.$user['user_first_name'].' '.$user['user_last_name'].'<br /><i class="fa fa-calendar"></i> '.$row['date_posted'].'</small></div>';
				} 
			}else{
...
//valid_input_simple
				...
				$attempt = 0;
				if($this->session->userdata('is_admin') == 1 || $this->session->userdata('user_role_id') == 3 || $this->session->userdata('user_id') == 8  ){
					$job_date = $this->input->post('job_date');
				}else{
