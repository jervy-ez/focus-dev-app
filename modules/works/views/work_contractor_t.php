		...
		<?php
			...
			<tr class="cont-<?php echo $row['works_contrator_id']; ?>" <?php if($is_selected == 1){ ?>style = "color: #CC6666"<?php } ?>>
				...
				<td class="item-cont-<?php echo $row['works_contrator_id']; ?>-exprce" align = right>
					<input type="text" onclick = "this.select()" onkeyup = "ku_update_exgst(<?php echo $row['works_contrator_id']; ?>)" onblur = "update_exgst(<?php echo $row['works_contrator_id']; ?>)" class="work-set-exgst-<?php echo $row['works_contrator_id']; ?> input_text text-right number_format price" value = "<?php echo number_format($row['ex_gst'],2) ?>" style = "width: 100%" <?php if($is_reconciled == 1){ ?>disabled<?php } ?>>
				</td>
				<td class="item-cont-<?php echo $row['works_contrator_id']; ?>-incprice" align = right>
					<input type="text" onclick = "this.select()" onkeyup = "ku_update_incgst(<?php echo $row['works_contrator_id']; ?>)" onblur = "update_incgst(<?php echo $row['works_contrator_id']; ?>)" class="work-set-incgst-<?php echo $row['works_contrator_id']; ?> input_text text-right number_format price" value = "<?php echo number_format($row['inc_gst'],2) ?>" style = "width: 100%" <?php if($is_reconciled == 1){ ?>disabled<?php } ?>>
				</td>
			</tr>
			<?php
			}
