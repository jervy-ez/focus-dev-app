						...
						<div class="col-md-12">
							<div class="left-section-box po">
										...
										<div class="col-lg-8 col-md-12">
											<div class="pad-left-15 pad-right-10 clearfix box-tabs">	
												<ul id="myTab" class="nav nav-tabs pull-right">
													<li class="">
														<a href="<?php echo base_url(); ?>purchase_order" ><i class="fa fa-level-up fa-lg"></i> View Outstanding</a>
													</li>
													<li class="active">
								...
								<div class="box-area">
									...
									<div class="box-tabs m-bottom-15">
										<div class="tab-content">
											...
											<div class="tab-pane  clearfix active" id="reconciled">
												...
												<div class="box-area po-area">
														<table id="reconciled_list_table" class="table table-striped table-bordered" cellspacing="0" width="100%">
															<thead><tr><th>PO Number</th><th>Project Number</th><th>CPO Date</th><th>Job Description</th><th>Contractor</th><th>Project Name</th><th>Reconciled Date</th><th>Client</th><th>Project Manager</th><th>Price</th><th>Balance</th><th>cpo_tmpstp_date</th></tr></thead>
                              <tbody>
                              ...
                              <?php
																...
																$r_total_price_exgst = 0;
																$r_total_price_incgst = 0;
                                  ...
                                  foreach ($reconciled_list->result_array() as $row){
                                    ...
                                    echo '<tr class="'.$row['contractor_type'].'_ct_type"><td><a href="#" data-toggle="modal" data-target="#reconciliated_po_modal" data-backdrop="static" id="'.$row['works_id'].'-'.$row['project_id'].'" onclick="return_outstanding_po_item(\''.$row['works_id'].'-'.$row['project_id'].'\');" class="return_outstanding_po_item">'.$row['works_id'].'</a></td><td><a href="'.base_url().'projects/view/'.$row['project_id'].'" >'.$row['project_id'].'</a></td><td>'.$row['work_cpo_date'].'</td><td><a href="'.base_url().'works/update_work_details/'.$row['project_id'].'/'.$row['works_id'].'">';
                                    ...
                                    $r_total_price_exgst = $row['price'] + $r_total_price_exgst;

                                    $r_inc_gst_price = $this->purchase_order->ext_to_inc_gst($row['price'],$prj_defaults['admin_gst_rate']);

                                    $r_total_price_incgst = $r_inc_gst_price + $r_total_price_incgst;
                                ...
                                ?>
                                  ...
                                  foreach ($reconciled_list_joinery->result_array() as $row_j){
                                    ...
                                    echo '<tr  class="'.$row_j['contractor_type'].'_ct_type" ><td><a href="#" data-toggle="modal" data-target="#reconciliated_po_modal" data-backdrop="static" id="'.$row_j['works_id'].'-'.$row_j['work_joinery_id'].'/'.$row_j['project_id'].'" onclick="return_outstanding_po_item(\''.$row_j['works_id'].'-'.$row_j['work_joinery_id'].'/'.$row_j['project_id'].'\');" class="select_po_item">'.$row_j['works_id'].'-'.$row_j['work_joinery_id'].'</a></td><td><a href="'.base_url().'projects/view/'.$row_j['project_id'].'" >'.$row_j['project_id'].'</a></td><td>'.$row_j['work_cpo_date'].'</td><td><a href="'.base_url().'works/update_work_details/'.$row_j['project_id'].'/'.$row_j['works_id'].'">';
...
<div style="display:none;" class="outstading_pm">
	...
	<div class="input-group  pull-right m-left-10"   style="width:180px;">
		<span class="input-group-addon" id="">View List</span>

		<select id="limiter" class="form-control  input-sm " onchange="limiter_load(this.value)">
			<option selected value="2000">2000</option>
			<option value="2500">2500</option>
			<option value="3000">3000</option>
			<option value="3500">3500</option>
			<option value="1000000">View All</option>
		</select>

		<script type="text/javascript">
			var limiter = <?php echo (isset($_GET['limiter']) ? $_GET['limiter'] : 2000 ); ?>;
			$('select#limiter').val(limiter);
		</script>
	</div>
...
<div class="po_legend_r hide">
	<p class="pad-top-5 m-left-10"> &nbsp;  &nbsp; <span class="ex-gst">$<?php echo number_format($r_total_price_exgst,2); ?>Ex GST</span> &nbsp;  &nbsp; <span class="inc-gst">$<?php echo number_format($r_total_price_incgst,2); ?> Inc GST</span></p>
</div>

<script type="text/javascript">
	function limiter_load(value){
		$('#loading_modal').modal({"backdrop": "static", "show" : true} );
		setTimeout(function(){
			window.location.assign("?limiter="+value);
		},1000);
  }
</script>
<style type="text/css">
