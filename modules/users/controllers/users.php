	...
	public function generate_leave_form($leave_req_id, $leave_user_id){
		...
		if ($partial_day == 1){
			...
			if ($leave_type_id >= 5){
				...
				$total_leave_pdf = $total_days_away / $no_hrs_of_work - $holiday_leave.' day(s)';
			}
		}
		...
		if ($leave_type_id < '5'){
			...
			if ($holiday_leave == 0){
				...
				$total_leave_holiday_days = round($total_leave / $no_hrs_of_work + $holiday_leave, 2);
				$holiday_hrs = $holiday_leave * $no_hrs_of_work;
				$total_leave_holiday_hrs = $total_leave + $holiday_hrs;
