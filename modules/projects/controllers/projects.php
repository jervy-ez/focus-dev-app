...
//		
	...
	public function progress_reports() {
		...
		if($q_proj->num_rows > 0){
			...
			$q_contact_person_email = $this->company_m->fetch_email($data['contact_person_id']);
			$contact_person_email = array_shift($q_contact_person_email->result_array());

			$data['contact_person_email'] = $contact_person_email['general_email'];
		...
		//	$data['postal_address_id'] = $company_detail['postal_address_id'];
			...
			$const_mngr_option = $this->user_model->fetch_user_by_role_with_number(11);
	...
	public function edit_image_label(){
		...
		$image_label = ucwords($image_data[0]);
		...
		$fetch_label_image = $q_fetch_label_image->result();

		if (!empty($fetch_label_image)){
						
			foreach ($fetch_label_image as $row){

				echo $row->image_label;
			}
		}
	...
	public function send_pr_pdf(){
		...
		$body_content .= '<span style="font-family: "Calibri, Candara, Segoe, Segoe UI, Optima, Arial, sans-serif"><br><br><br><br>Regards,<br><br><strong>'.$this->session->userdata('user_first_name').' '.$this->session->userdata('user_last_name').'</strong><br><br>'.$this->session->userdata('role_types').'<br><span style="color: "#FF3399";>'.$sendpdf_from.'</span></span><br><br>';
	...
	public function getProject_PR_images(){
		...
		if ($user_if_admin['if_admin'] == '1'){
			...
			} elseif ($user_if_admin['user_role_id'] == '3') { // PM
	    ...
	    for($i=0; $i<$cpt; $i++)
	    {
		    ...
		    $this->upload->initialize($this->set_upload_options($project_id));
		   	...
		   	if ( !$this->upload->do_upload()) {
		   		...
		   		$this->session->set_flashdata('pr_images', 'Successfully uploaded an image.');
				$this->projects_m->insert_pr_images($project_id,$image_path);//$_FILES['userfile']['name']);
		   	}
