												...
												<div class="col-md-5 col-sm-12  col-xs-12" id="">
													<div class="pad-right-5 pad-left-5">
														...
														<table id="dataTable_noCustom" class="table table-striped table-bordered table-condensed" cellspacing="0" width="100%">
															<thead> <tr><th>Year</th> <th>Type</th>  <th>Data Name</th> <th>Company</th> <th>Total</th></tr> </thead>
															<tbody>
																<?php foreach ($stored_revenue_forecast as $key => $value): ?>
																	<tr id="<?php echo $value->revenue_forecast_id; ?>">
																		<td><a href="#" class="stored_rfc" id="<?php echo number_format($value->jul_breakdown,2).'-'.number_format($value->aug_breakdown,2).'-'.number_format($value->sep_breakdown,2).'-'.number_format($value->oct_breakdown,2).'-'.number_format($value->nov_breakdown,2).'-'.number_format($value->dec_breakdown,2).'-'.number_format($value->jan_breakdown,2).'-'.number_format($value->feb_breakdown,2).'-'.number_format($value->mar_breakdown,2).'-'.number_format($value->apr_breakdown,2).'-'.number_format($value->may_breakdown,2).'-'.number_format($value->jun_breakdown,2); ?>"><?php echo $value->year; ?></a></td>
