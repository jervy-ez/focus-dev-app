...
*/
	...
	public function dash_unvoiced_per_date($date,$comp_id){
		$query = $this->db->query("SELECT `invoice`.`project_id`, `invoice`.`invoice_date_req`, `invoice`.`set_invoice_date`, `project`.`project_total`,`project_cost_total`.`variation_total`, `invoice`.`progress_percent`, `invoice`.`label` ,`project`.`focus_company_id`
			FROM `invoice`
			LEFT JOIN `project` ON `project`.`project_id` = `invoice`.`project_id`
			LEFT JOIN `project_cost_total` ON `project_cost_total`.`project_id` = `invoice`.`project_id`
			WHERE `project`.`job_date` <> ''
			AND UNIX_TIMESTAMP( STR_TO_DATE(`invoice`.`invoice_date_req`, '%d/%m/%Y') ) < '$date'
			AND `project`.`is_active` = '1' AND  `invoice`.`is_invoiced` = '0' AND `project`.`is_paid` = '0' 
			AND `project`.`focus_company_id` = '$comp_id' ");
		return $query;
	}

	public function dash_oustanding_payments(){
		$query = $this->db->query("SELECT `project`.`focus_company_id`,SUM( `payment`.`amount_exgst`) AS `payment_amount`
			FROM `invoice`
			LEFT JOIN `project` ON `project`.`project_id` = `invoice`.`project_id`
			LEFT JOIN `payment` ON `payment`.`invoice_id` = `invoice`.`invoice_id`
			WHERE `project`.`job_date` <> ''
			AND `project`.`is_active` = '1' AND  `invoice`.`is_invoiced` = '1' AND `project`.`is_paid` = '0' AND  `invoice`.`is_paid` = '0'
			GROUP BY `project`.`focus_company_id`  ORDER BY `project`.`focus_company_id` ASC");
		return $query;
	}
