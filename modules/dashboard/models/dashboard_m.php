...
*/
	...
	public function get_site_labour_hrs($currect_date){
		...
		$query = $this->db->query("SELECT  UNIX_TIMESTAMP( STR_TO_DATE(`project_labour_sched`.`labour_sched_date`, '%Y-%m-%d') ) AS `date_set`, SUM(`project_labour_sched`.`number_of_hrs`) AS `time` , `project`.`focus_company_id`
			FROM `project_labour_sched`
			LEFT JOIN `project` ON `project`.`project_id` = `project_labour_sched`.`project_id`
			WHERE UNIX_TIMESTAMP( STR_TO_DATE(`project_labour_sched`.`labour_sched_date`, '%Y-%m-%d') ) > UNIX_TIMESTAMP( STR_TO_DATE('$currect_date', '%Y-%m-%d') ) /* YYYY-MM-DD */
