...
// 			echo "<p>----</p>";
		...
		if($current_standing > $forecasted_amount){
			...
			if($forecasted_amount > 0){
				...
				echo '<div class="y progress-bar progress-bar-danger active progress-bar-striped tooltip-enabled" data-html="true" data-placement="bottom" data-original-title="Current Standing plus YTD WIP<br />$'.number_format($current_standing).' at 100%"  style="left: 30px; position: absolute; width: 93.5%; background-color: #1c61a7; border-radius: 20px; height: 30px; text-align: right; padding-right: 10px; ">100%</div>';
...
*/
	...
	public function get_count_maintenance(){
		...
		//echo "<p>$this_year_date,$this_year_end-----$last_year_date,$last_year_end</p>";


		$count_m_prj_aq = $this->dashboard_m->count_maintenance_projects($this_year_date,$this_year_end);
		$count_m_prj_a = array_shift($count_m_prj_aq->result());
		$limit = 0;


		$count_m_prj_bq = $this->dashboard_m->count_maintenance_projects($last_year_date,$last_year_end);
		$count_m_prj_b = array_shift($count_m_prj_bq->result());

		//var_dump($count_m_prj_a->prj_numbers);
		//var_dump($count_m_prj_b->prj_numbers);
...
//		$company_id = 6;
		...
		$current_forecast_fc = $q_get_current_forecast->result();
		$current_forecast_fcur = array_shift($current_forecast_fc);


		if($q_get_current_forecast->num_rows >= 1){
			$forecasted_amount =  $current_forecast_fcur->total * ($current_forecast_fcur->forecast_percent/100) ;
			//var_dump($current_forecast_fcur->total);
			//var_dump($current_forecast_fcur->forecast_percent);
		}
...
// --------------------------------------------
	...
	public function management_financial_year_report(){
		$is_company = 0;
		...
		if($is_company == 1 && $comp_id_set > 0){
			...
			$q_focus_pm_forecast_old = $this->dashboard_m->get_pm_forecast($report_year-1,'',$comp_id_set);
			...
			$pms_sales_last_year_q_older = $this->dashboard_m->fetch_pm_sales_year($report_year-2,'',$comp_id_set);
			$pms_sales_last_year_q_oldest = $this->dashboard_m->fetch_pm_sales_year($report_year-3,'',$comp_id_set);



			$focus_data_forecast_p = $this->dashboard_m->get_focus_comp_forecast($report_year,$comp_id_set);
			$focus_data_forecast_p_old = $this->dashboard_m->get_focus_comp_forecast($report_year-1,$comp_id_set);
		}elseif($comp_id_set == 0){

			$data['company_details'] = '';

			$q_focus_pm_forecast = $this->dashboard_m->get_pm_forecast($report_year);
			$q_focus_pm_forecast_old = $this->dashboard_m->get_pm_forecast($report_year-1);
			...
			$pms_sales_last_year_q_older = $this->dashboard_m->fetch_pm_sales_year($report_year-2,'','',1);
			$pms_sales_last_year_q_oldest = $this->dashboard_m->fetch_pm_sales_year($report_year-3,'','',1);

			 
			$focus_data_forecast_p = $this->dashboard_m->get_focus_comp_forecast($report_year);
			$focus_data_forecast_p_old = $this->dashboard_m->get_focus_comp_forecast($report_year-1);
		...
		//	$focus_data_forecast_p = $this->dashboard_m->get_focus_comp_forecast($report_year);
			...
			$q_focus_pm_forecast_old = $this->dashboard_m->get_pm_forecast($report_year-1,$pm_id);
			...
			$pms_sales_last_year_q_older = $this->dashboard_m->fetch_pm_sales_year($report_year-2,$pm_id);
			$pms_sales_last_year_q_oldest = $this->dashboard_m->fetch_pm_sales_year($report_year-3,$pm_id);
		...
		$data['pm_forecast_old'] = array_shift($q_focus_pm_forecast_old->result_array());
		...
		$data['pm_actual_sales_old'] = array_shift($pms_sales_last_year_q_older->result_array());
		$data['pm_last_year_sales_old'] = array_shift($pms_sales_last_year_q_oldest->result_array());

		$q_focus_data_forecast_p = $focus_data_forecast_p->result();
		$q_focus_data_forecast_p_old = $focus_data_forecast_p_old->result();
		...
		foreach ($q_focus_data_forecast_p_old as $ffcp){
			$forecast_percent_val = $ffcp->total * ( $ffcp->forecast_percent / 100 );
			$forecast_per_comp[$ffcp->comp_id] = $forecast_percent_val;
		}
		$data['focus_comp_forecast_old'] = $forecast_per_comp;


		$this->load->view('dashboard_report_financial', $data);
	}
...
//		redirect('/dashboard/sales_forecast', 'refresh');
	...
	public function test_data_uninvoiced_list(){
...
ORDER BY `project`.`project_id` ASC ");
		...
		foreach ($dash_unvoiced as $unvoiced) {
			...
			echo "$unvoiced->project_id".'_'."$unvoiced_total_init<br/>";
...
foreach ($result_q as $data) {
	...
	echo "$data->project_id-".$prj_data['gp']."<br/>";
...
/*		$this->db->query(" UPDATE `user_access` SET `client_supply` = '2' WHERE `user_access`.`user_access_id` = '86';
		 	UPDATE `user_access` SET `client_supply` = '2' WHERE `user_access`.`user_access_id` = '13';
		 	UPDATE `user_access` SET `client_supply` = '2' WHERE `user_access`.`user_access_id` = '12';
		 	UPDATE `user_access` SET `client_supply` = '2' WHERE `user_access`.`user_access_id` = '7';
		 	UPDATE `user_access` SET `client_supply` = '1' WHERE `user_access`.`user_access_id` = '14';
		 	UPDATE `user_access` SET `client_supply` = '2' WHERE `user_access`.`user_access_id` = '3'; ");
