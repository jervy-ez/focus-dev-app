...
// temporary restriction for dashboaed dev
		...
		if($this->session->userdata('is_admin') == 1 || $user_role_id == 16 || $user_role_id == 3 || $user_role_id == 2 || $user_role_id == 7):
		else:		
...
//		$this->users->_check_user_access('dashboard',1);
	...
	//	redirect('projects', 'refresh');
		...
		foreach ($post_months as $key => $value) {
			//echo "$key => $value<br />";
			$swout['sales_data_'.$value] = $sales_focus_company['rev_'.$value];
	...
	public function wid_quoted(){
		...
		foreach ($focus_company as $company){
			if($quoted_focus_company[$company->company_id] > 0){
				$total_string .= '<div class=\'row\'><span class=\'col-xs-6\'>'.str_replace("Pty Ltd","",$focus_arr[$company->company_id]).'</span> <span class=\'col-xs-6\'>$ '.number_format($cost_focus[$company->company_id],2).'</span></div>';
			}
		}
		$total_string .= '<div class=\'row\'><span class=\'col-xs-12\'><hr style=\'margin:4px 0px;\' /></span></div>';

		foreach ($project_manager_list as $pm ) {
			$total_string .= '<div class=\'row\'><span class=\'col-xs-6\'>'.$quoted_pm_name[$pm->user_id].'</span> <span class=\'col-xs-6\'>$ '.number_format($cost_pm[$pm->user_id],2).'</span></div>';
		}

		$total_string .= '<div class=\'row\'><span class=\'col-xs-12\'><hr style=\'margin:4px 0px;\' /></span></div>';

		foreach ($estimator_list as $est ) {
			$total_string .= '<div class=\'row\'><span class=\'col-xs-6\'>'.$quoted_estimator_name[$est->project_estiamator_id].'</span> <span class=\'col-xs-6\'>$ '.number_format($cost_estimator[$est->project_estiamator_id],2).'</span></div>';
		...
		echo '<p class="value tooltip-enabled" title="" data-html="true" data-placement="bottom" data-original-title="'.$total_string.'"><strong><i class="fa fa-usd"></i> '.number_format(array_sum($cost_focus),2).'</strong></p>';
	}
	...
	public function average_date_invoice_pa(){
 	...
 	//	foreach ($direct_company as $key => $comp_id) {
			...
			foreach ($days_result as $result){
				array_push($days_dif, $result->days_diff);
			}
		...
		//	if($pm_type == 1){ // for director/pm
				foreach ($project_manager_list as $pm ) {
					...
					foreach ($days_result_pm as $result_pm){
						array_push($pms_averg[$pm->user_id],$result_pm->days_diff);
					}
				}
				}
		//	}
			...
			if( $company->company_id == $user_details['user_focus_company_id'] ){
				...
				foreach ($days_result as $result){
					if($result->project_manager_id != 9){
						array_push($days_dif_comp, $result->days_diff);
					}
				}
...
*/
	...
	/*	foreach ($focus_company as $company){
			if($company->company_id != 4){
				...
				$q_ave_c = $this->dashboard_m->get_maitenance_dates_pm($date_a,$date_b,$company->company_id); //2
				$days_result_c = $q_ave_c->result();

				foreach ($days_result_c as $result){
					array_push($days_dif_comp, $result->days_diff);
				}

				$size_c = count($days_dif_comp);
				$average_comp = array_sum($days_dif_comp) / $size_c;
...
/*
		foreach ($project_manager_list as $pm ){
			...
			$q_ave_pm = $this->dashboard_m->get_maitenance_dates_pm($date_a,$date_b,$pm->user_id); //3
			$days_result_pm = $q_ave_pm->result();

			foreach ($days_result_pm as $result){
				array_push($days_dif_pm, $result->total_days);
			}

			$size = count($days_dif_pm);

			if($size > 0){
				$pm_average = array_sum($days_dif_pm) / $size;
				arsort($days_dif_pm,1);
				$pm_long_day =  max($days_dif_pm);
				$pm_short_day_day =  min($days_dif_pm);
				$pm_short_day_day = 1;
			}else{
				$pm_average = 0;
				$pm_long_day = 0;
