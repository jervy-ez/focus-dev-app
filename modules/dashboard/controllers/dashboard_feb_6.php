...
*/
	...
	public function focus_projects_count_widget_mn(){
		...
		$lat_old_year = $year-1;
		$total_string_inv .= '<div class=\'row\'><span class=\'col-xs-12\'><hr style=\'margin:4px 0px;\' /></span> &nbsp; Last Year '.$lat_old_year.'</div>';
...
*/
	...
	public function focus_get_po_widget(){ 
		...
		$po_list_ordered = $this->purchase_order_m->get_po_list_order_by_project($current_start_year,$date_last_year_today);
		foreach ($po_list_ordered->result_array() as $row){
			$work_id = $row['works_id'];
...
echo "['". str_replace("'","&apos;",$comp_name)."', ".$total_price."],";
	...
	public function maintanance_average(){
		...
		echo '<p class="value tooltip-enabled" title="" data-html="true" data-placement="bottom" data-original-title="Last Year '.$last_year.' : '.number_format($average_old,2).'">'.number_format($average,2).' Days';
		echo '<span class="pull-right">'.$short_day_day.'  <i class="fa fa-arrows-h" aria-hidden="true"></i> '.$long_day.'</span></p>';
	}
