...
*/
		...
		foreach ($comp_q->result_array() as $row){
			...
			if($send_status == 0){
				...
				if($has_insurance_income_protection == 1){
					$income_protection_expiration = $row['income_protection_expiration'];
					$ipe_arr =  explode('/',$workers_compensation_expiration);
					$ipe_day = $ipe_arr[0];
					$ipe_month = $ipe_arr[1];
					$ipe_year = $ipe_arr[2];
					$ipe_date = $ipe_year.'-'.$ipe_month.'-'.$ipe_day;
	...
	public function contractors_list(){
		$comp_q = $this->company_m->fetch_all_company_details();
		$date = date('Y-m-d');
		echo '<table width = "100%" class ="table table-striped" table-bordered><th width = 20px></th><th>Company Name</th>';
		foreach ($comp_q->result_array() as $row){
			$insurances = array();
			$expired = 0;
			$send_status = $row['email_send_status'];
			...
			if($expired > 0){
				echo '<tr><td><input type="checkbox" name = "chk_contractor_list" value = "'.$company_id.'"></td><td>'.$company_name.'</td></tr>';
			}
		}
		echo '</table>';
	}
