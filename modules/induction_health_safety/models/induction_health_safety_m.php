...
*/
	...
	public function list_uploaded_files_arch($type_id,$view_old=0){

		if($view_old == 1){
			$query = $this->db->query("SELECT * FROM `archive_documents` `ad_main`
				WHERE `ad_main`.`date` = ( SELECT `archive_documents`.`date` FROM `archive_documents` WHERE `archive_documents`.`type_id` = '$type_id' GROUP BY `archive_documents`.`date` ORDER BY `archive_documents`.`archive_documents_id` DESC LIMIT 1,1 )
