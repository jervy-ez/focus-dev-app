...
*/
	...
	function override_sched(){
		...
		$user_id = $this->session->userdata('user_id'); //this is a dummy test user
		$my_time_log = $this->functions->get_time_record();

		$arr_days = explode(',', $days);

		$q_course_schedule = $this->scheduling_m->_get_course_schedule($course_id,$faculty_id,$term_id);
		...
		if($course_schedule){
			...
			$course_schedule_id = $this->scheduling_m->_insert_course_schedule($course_id, $block_id, $class_faculty_id, $term_id, $my_time_log, $user_id);
		}
	...
	//	$user_id = '1314';
		...
		//$schedule_name = ''; // to be fetched
		//$no_of_units = '0'; // to be fetched
		$has_conflict = 0;
	...
	list($days, $room_id, $start_time, $end_time, $faculty_id, $course_id, $block_id, $term_id, $class_event, $class_faculty_id, $schedule_name,$no_of_units) = $arr_post;
		...
		// days+'|'+select_room+'|'+start_time+'|'+end_time+'|'+select_faculty+'|'+course_id+'|'+class_block+'|'+term+'|'+class_event+'|'+select_class_faculty+'|'+schedule_name;
/*
'days'+'|'+select_room+'|'+start_time+'|'+end_time+'|'+select_faculty+'|'+course_id+'|'+class_block+'|'+term+'|'+class_event+'|'+select_class_faculty+'|'+schedule_name;

Mon   +     180       +      835     +   950      +         325      +     1231    +       2       +    1   +        1      +             325        +   shed 1
*/
...
if($room_id == 0){
	$room_id = 'NULL';
		...
		if($course_schedule){
			...
			// gets the course_schedule_id after insert
			// echo "<br /><br />$course_id, $block_id, $class_faculty_id, $term_id, $my_time_log, $user_id<br /><br />";
			// $block_id,
			$course_schedule_id = $this->scheduling_m->_insert_course_schedule($course_id, $class_faculty_id, $term_id, $my_time_log, $user_id);
		
		if($block_id >= 1){
			$this->scheduling_m->_insert_class_block($course_schedule_id, $block_id, $my_time_log, $user_id);
		}
	...
	function _check_time_conflict($query_result,$room_id,$faculty_id,$block_id,$start_time,$end_time,$days,$type){
		...
		if($query_result){
			...
			//	echo "<br /><hr /><br />";
				...
				$index_end = $class_data->year_level;
				$block_year = $class_data->year_level;
				$block_code = $class_data->block_code;
				...
				if( intval($class_data->start_time) <= intval($start_time) && intval($start_time) < intval($class_data->end_time )){
					...
					$message = "<p><strong>$type Conflict</strong> : ".$day_arr[$stored_day]." on $class_data->time at Room:$data_room_name by $data_faculty_name, Class ".$block_year."".$ends[$index_end]." year block-".$block_code."</p>";
...
// echo "$has_conflict*";
	...
	function set_schedule(){
		...
		//166-3-3-3-4
		list($course_id, $campus_id, $college_id, $department_id, $department_program_id) = $course_arr;
