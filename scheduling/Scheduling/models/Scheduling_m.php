...
class Scheduling_m extends CI_Model{
	...
	function _get_course($year_term = ''){
		$query = $this->db->query("SELECT *, course.id AS class_subject_id, program.id AS program_id, campus.id AS campus_id, college.id AS college_id, department.id AS department_id   
			FROM curriculum 
...
*/
	...
	function _get_course_schedule($course_id, $faculty_id, $term_id,$block_id){
		$query = $this->db->query("SELECT * FROM course_schedule  
			LEFT JOIN course_schedule_block ON course_schedule_block.course_schedule_id = course_schedule.id
			WHERE course_schedule.course_id = '$course_id' 
			AND course_schedule.faculty_in_charge_id = $faculty_id 
			AND course_schedule_block.block_id = '$block_id'
	...
	function _insert_course_schedule($course_id,  $faculty_id, $term_id, $date_time_log , $user_id){
		$this->db->query("INSERT INTO course_schedule (course_id, faculty_in_charge_id, academic_term_id, active, is_active, created_on, created_by, modified_on, modified_by)
		 VALUES ('$course_id',  $faculty_id, '$term_id', 'T', 'T', '$date_time_log', '$user_id', '$date_time_log', '$user_id'  )");
	...
	function _get_class_schedule($id,$term_id,$days,$type=''){
		...
		if($type != '' && $type == '1'){ // 1 = seach by faculty
			$query = $this->db->query("SELECT * FROM class_schedule 
				...
				LEFT JOIN course_schedule_block ON course_schedule_block.course_schedule_id = course_schedule.id
				LEFT JOIN block ON block.id = course_schedule_block.block_id
	...
	function _get_block_schedule($id,$term_id){
		$query = $this->db->query("SELECT * , class_schedule.id AS class_sched_id FROM class_schedule 
			...
			LEFT JOIN course ON course.id = course_schedule.course_id
			LEFT JOIN course_schedule_block ON course_schedule_block.course_schedule_id = course_schedule.id
			WHERE course_schedule_block.block_id = '$id'  AND course_schedule.academic_term_id = '$term_id'
			AND class_schedule.active = 'T' AND class_schedule.is_active = 'T' ORDER BY class_schedule.start_time ASC");
		return $query;
	}
	...
	function _get_department($id){
		$query = $this->db->query("SELECT *, college_department.id AS college_dep_id FROM college_department LEFT JOIN department ON department.id = college_department.department_id
			WHERE college_department.college_id= '$id' /*AND college_department.is_active = 'T' AND department.is_active = 'T'*/ ");
		return $query;
	}

	function _get_program($id){
		$query = $this->db->query("SELECT *, program.id AS program_id FROM department_program LEFT JOIN program ON program.id = department_program.program_id
			WHERE department_program.college_department_id = '$id' /*AND department_program.is_active = 'T' AND program.is_active = 'T'*/ ");
		return $query;
	}
...
*/
	...
	function _get_schedules(){
		$query = $this->db->query("SELECT *,  class_schedule.id AS class_sched_id FROM class_schedule
			...
			LEFT JOIN course_schedule ON course_schedule.id = class_schedule.course_schedule_id
			LEFT JOIN course_schedule_block ON course_schedule_block.course_schedule_id = class_schedule.course_schedule_id
			LEFT JOIN block ON block.id = course_schedule_block.block_id
			LEFT JOIN course ON course.id = course_schedule.course_id
	...
	function _disable_schedule($id){
		$query = $this->db->query("UPDATE class_schedule SET active = 'F' , is_active = 'F' WHERE id = '$id' ");
		return $query;
	}

/**/
			/*AND block.is_active = 'T'*/
}
